# Script: Perform model estimation of RTGP with variational inference (for right hemisphere, for various kernel hyperparmeter settings)
...
L = 800
# Vector of bandwidth options
phi_list = c(0.01, 0.05, 0.1, 0.2, 0.3)
phi = phi_list[sim]
# Vector of exponent options
nu_list = c(rep(c(1.2, 1.4), 5))
nu = nu_list[sim]
...
X = fread(paste0(path_data, 'X_right_cortex_train.csv'))
...
load(paste0(path_init, "/result_right_cortex.RData"))
beta0_NM = result_right_cortex$nm_fit_SOI$post_mean$betacoef[1,]
beta_confound_NM = result_right_cortex$nm_fit_SOI$post_mean$betacoef[2:(P+1),]
beta_cortex_NM = result_right_cortex$beta_fit$NM
theta_NM = result_right_cortex$theta_fit$NM
sigma_eps2_NM = result_right_cortex$nm_fit_SOI$post_mean$sigma2_eps
...
while((diff > eps) && (iter != n_iter)){
  tryCatch({
    ...
    if(iter %% 10 == 0){
      ...
      save(result, file = sprintf("%sresult_rtgp_vi_right_cortex.RData", path_out))
      rm(result)
