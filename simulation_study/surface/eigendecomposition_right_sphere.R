# Script: Perform eigendecomposition of kernel matrix for right hemisphere of the brain (for various kernel hyperparameters
...
path = ''
path_out = paste0(path, 'eigendecomposition_R', toString(L), '_phi', toString(phi), '_nu', toString(nu), '/')
dir.create(file.path(path_out), showWarnings = FALSE)

# Distance matrix 
distance_matrix = data.matrix(fread(paste0(path, 'RightDenseGeodesicDistmat.txt')))

if(dim(distance_matrix)[1] != dim(distance_matrix)[2]){
  distance_matrix = distance_matrix[,2:dim(distance_matrix)[2]]
}

mask = as.numeric(unlist(fread('/well/nichols/users/fxo071/IOI-GP/sim_study/11.21.23/data/idx_right.csv')[,2]))
...
write.csv(eigendecomp$values, paste0(path_out, 'eigenvalues_right_cortex.csv'))
write.csv(eigendecomp$vectors, paste0(path_out, 'eigenvectors_right_cortex.csv'))
save(eigendecomp, file = paste0(path_out, 'eigendecomposition_right_cortex.RData'))
