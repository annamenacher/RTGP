# Script: Sensitivity analysis of variance hyperparameters with model estimation of GPR + Normal, GPR + Horseshoe, and RTGP-VI

options(bitmapType='cairo-png')
dataset = 1
sim = as.numeric(Sys.getenv("SLURM_ARRAY_TASK_ID"))

var_hyperparam_beta_list = c(0.01, 0.001, 0.0001)
var_hyperparam_alpha_list = b_alpha = c(0.01, 0.001, 0.0001)
var_hyperparam_epsilon_list = b_epsilon = c(0.01, 0.001, 0.0001)
...
path_results_NM =  sprintf("%s/N%s_L%s_phi%s_nu%s/dataset%s/GPR/", path_core, n_train, L, phi, nu, dataset)
dir.create(file.path(path_results_NM), showWarnings = FALSE)

path_results_RTGP_VI =  sprintf("%s/N%s_L%s_phi%s_nu%s/dataset%s/RTGP_VI_sensitivity/", path_core, n_train, L, phi, nu, dataset)
...
a_beta = b_beta = var_hyperparam_beta_list[sim]
a_alpha = b_alpha = var_hyerparam_alpha_list[sim]
a_epsilon = b_epsilon = var_hyperparam_epsilon_list[sim]
...
row_name = c('RTGP', 'GPR + Normal', 'GPR + Horseshoe')
row_name_inference = c('RTGP', 'GPR + Normal', 'GPR + Horseshoe')
...
matrix_inference = matrix(NA, 3, 4)
...
matrix_parameters = matrix(NA, 3, 2)
...
matrix_prediction = matrix(NA, 3, 4)
