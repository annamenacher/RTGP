# Script: Model estimation of GPR + Normal, GPR + Horseshoe, and RTGP-VI for various initialisations (check that ELBO converges reliably)
...
path_results_RTGP_VI =  sprintf("%s/N%s_L%s_phi%s_nu%s/dataset%s/RTGP_VI_ELBO/", path_core, n_train, L, phi, nu, dataset)
...
beta0 =  nm_fit_SOI$post_mean$betacoef[1]+ rnorm(1, 0, 0.01)
beta = beta_fit$NM + rnorm(M,0, 0.01)
sigma_epsilon = sqrt(nm_fit_SOI$post_mean$sigma2_eps) + abs(rnorm(1,0, 0.01))
sigma_alpha = sigma_alpha_prior =  runif(1,0.01,1)
sigma_beta = sigma_beta_prior =  runif(1,0.01,1)
